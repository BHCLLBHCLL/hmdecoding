set f [open "output/ground_truth/hook_tail.txt" w]
proc logit {msg} { global f; puts $f $msg; flush $f }
catch {*readfile "C:/Program Files/Altair/2019/tutorials/hm/interfaces/samcef/hook.hm" 1} rerr
logit "read: $rerr"
catch {*createmark elements 1 "all"} _
set ids [hm_getmark elements 1]
logit "elems=[llength $ids] last=[lrange $ids end-9 end]"
foreach id $ids {
    if {$id >= 12103 && $id <= 12115} {
