set f [open "output/ground_truth/keyhole_tail.txt" w]
proc logit {msg} { global f; puts $f $msg; flush $f }
catch {*readfile "C:/Program Files/Altair/2019/tutorials/hm/keyhole.hm" 1} rerr
logit "read: $rerr"
catch {*createmark elements 1 "all"} _
set ids [hm_getmark elements 1]
logit "elems=[llength $ids] last=[lrange $ids end-5 end]"
foreach id $ids {
    if {$id >= 500 && $id <= 505} {
