...
got = parse_a_type(p, segs[0][0] + 24, segs[0][3], ncount, row_map, segs[0][2] - 71)
gt = json.load(open("output/ground_truth/multi_elem_gt.json"))["clip_refine.hm"]
print("parsed keys:", sorted(got.keys()))
for eid in [1, 2, 3]:
    g = gt["elems"][str(eid)]
    exp = [x for x in g["nodes"] if x]
    mine = got[eid]
    print(f"E{eid}: mine={mine} gt_cfg={g['cfg']} gt_nodes={g['nodes']} exp={exp}")
    print(f"   match? cfg={mine[0] == g['cfg']} nodes={mine[1][:len(exp)] == exp}")
