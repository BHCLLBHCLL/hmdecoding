        ...
        for k in range(min(cnt, max_rec)):
            ...
            for j in range(rec + 24, min(rec + 200, len(p) - 4)):
                if u32(p, j) == CONST:
                    ...
                    break
            cands = []
            lim = stride - 12 if stride else 84
            ...
            for fp in sorted(cands, reverse=True):
                ...
                if n < 1:
                    continue
                if stride is not None and nodes_off + 4 * n + 8 != rec + stride:
                    continue
                if u32(p, nodes_off + 4 * n) != 0 or u32(p, nodes_off + 4 * n + 4) not in (1, 2, 4):
                    continue
                nds = [u32(p, nodes_off + 4 * j) for j in range(n)]
                if all(1 <= r <= row_count for r in nds):
                    got = (n, nds, (u32(p, rec + fp) >> 16) - 256)
                    break
            if got is None:
                ok = False; break
            n, nds, config = got
