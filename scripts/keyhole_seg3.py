...
sh=[s[0] for s in segs if s[1]==3][0]
cnt=[s[3] for s in segs if s[1]==3][0]
print('seg3 @',sh,'cnt',cnt)
recs=[]; pos=sh+16; end=min(sh+400,len(p))
while pos<end:
    if is_const(u32(p,pos)): recs.append(pos)
    pos+=4
for k,cp in enumerate(recs[:6]):
    print(' rec%d @%d const=%08x words:'%(k,cp,u32(p,cp)), ' '.join('%08x'%u32(p,cp+w*4) for w in range(12)))