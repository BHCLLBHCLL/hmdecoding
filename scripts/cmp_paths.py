...
from decoder import _parse_b_type, _parse_b_slots
...
for sh, segid, cfg71, cnt, X, Y in segs:
    b = _parse_b_type(p, sh, cnt, ns[1], rm, Y)
    s2 = _parse_b_slots(p, sh, cnt, ns[1], rm, Y)
    print(f"seg{segid}: cnt={cnt} btype={len(b) if b else 0} slots={len(s2) if s2 else 0}")
