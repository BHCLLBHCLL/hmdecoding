...
CONST = 0x70241FF5
CONFIG_NODES = {103: 3, 104: 4, 204: 4, 220: 8, 100: 2, 101: 2, 102: 2, 105: 2, 106: 2, 108: 2, 112: 2, 114: 2, 201: 3, 202: 3, 203: 3, 205: 3, 206: 3, 301: 6, 302: 8, 303: 6, 304: 8, 305: 10, 306: 12}
...
def parse_b_type(p, sh, cnt, row_count, row_map, first_eid, maxprobe=400):
    flag = u16(p, sh + 32)
    if not (300 <= flag <= 500):
        return None
    config = flag - 256
    stride = None
    for j in range(sh + 24 + 22, min(sh + 24 + 22 + maxprobe, len(p) - 4)):
    ...
    for k in range(min(cnt, 10)):
        ...
        if not all(1 <= r <= row_count for r in nds):
            return None
        elems[eid] = (config, [row_map.get(r, r) for r in nds])
        # next eid: find u32 0 after nodes, then u16 pair (v, 0)
        z = None
        for zz in range(rec + 10 + 4 * n, rec + stride):
            if u32(p, zz) == 0:
                z = zz
                break
        if z is None:
            return None
        ne = u16(p, z + 4)
        if u16(p, z + 6) != 0 or ne == 0 or ne >= 10_000_000:
            return None
        eid = ne
    return elems
