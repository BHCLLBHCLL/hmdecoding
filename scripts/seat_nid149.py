...
for fn,label in [('C:/Program Files/Altair/2019/tutorials/hm/interfaces/lsdyna/seat_2.hm','seat_2')]:
    raw=open(fn,'rb').read()
    ...
    for k in range(count):
        ...
        if nid==149:
            ...
            print('%s: k=%d rec=%d nid=149 x=%g y=%g z=%g'%(label,k,rec,x,y,z))
            # neighbors
            for kk in (k-1,k+1):
                r2=base+kk*stride
                if 0<=r2 and r2+stride<=len(p):
                    ...
                    print('   k=%d nid=%d x=%g y=%g z=%g'%(kk,n2,x2,y2,z2))