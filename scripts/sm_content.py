...
def ck(path,ef,label,nfilter):
    ...
    print('%s: same_cfg %d same_nds %d/%d only_dec %d only_ora %d'%(label,sc,sn,len(oracle),od,oo))
valid=sorted(int(x.strip()) for x in open('output/ground_truth/sm_nodes_all.txt',encoding='utf-8') if x.strip().isdigit())
ck('C:/Program Files/Altair/2019/tutorials/hm/interfaces/lsdyna/SEAT_MODEL.hm','output/ground_truth/elems/SEAT_MODEL.hm.elems.txt','SEAT_MODEL(plain)',None)
ck('C:/Program Files/Altair/2019/tutorials/hm/interfaces/lsdyna/SEAT_MODEL.hm','output/ground_truth/elems/SEAT_MODEL.hm.elems.txt','SEAT_MODEL(strict)',set(valid))