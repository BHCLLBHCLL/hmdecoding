set f [open "output/ground_truth/abq_elem.txt" w]
proc logit {msg} { global f; puts $f $msg; flush $f }
catch {*readfile "C:/Program Files/Altair/2019/tutorials/hm/interfaces/abaqus/abaqus_contactManager_2D_tutorial.hm" 1} rerr
logit "read: $rerr"
catch {*createmark elements 1 "all"} _
set ids [hm_getmark elements 1]
logit "elems=[llength $ids] first=[lrange $ids 0 9] last=[lrange $ids end-9 end]"
foreach id $ids {
    if {$id >= 375 && $id <= 435} {
