...
CONST = 0x70241FF5
CONFIG_NODES = {103: 3, 104: 4, 204: 4, 220: 8, 100: 2, 101: 2, 102: 2, 105: 2, 106: 2, 108: 2, 112: 2, 114: 2, 201: 3, 202: 3, 203: 3, 205: 3, 206: 3, 301: 6, 302: 8, 303: 6, 304: 8, 305: 10, 306: 12, 401: 8, 402: 8, 403: 8, 404: 8, 405: 6, 406: 6, 407: 6, 408: 6, 409: 6, 410: 6}
...
def parse_a_type(p, s, cnt, row_count, row_map, maxprobe=200):
    stride = None
...
def parse_b_type(p, s, cnt, row_count, row_map, first_eid, maxprobe=400):
    flag = u16(p, s + 8)
    if not (300 <= flag <= 500):
        return None
    config = flag - 256
...
for fname, info in gt.items():
    ...
    for (sh, segid, cfg71, cnt, X, Y) in segs:
        got = None
        if X == 3:
            got = parse_a_type(p, sh + 24, cnt, ncount, row_map)
        else:
            got = parse_b_type(p, sh + 24, cnt, ncount, row_map, Y)
        if got is None:
