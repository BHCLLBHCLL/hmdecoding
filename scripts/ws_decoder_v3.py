...
for pid, (t, x, y, z) in res.items():
    if pid in pts:
        ...
        if max(abs(x - xo), abs(y - yo), abs(z - zo)) < 2e-3:
            ...
            bad.append(pid)
out.append(f"v3: {ok}/{len(res)} 正确 ({100*ok//len(res)}%), 错 {len(bad)}")
out.append(f"错误: {bad[:12]}")
open("output/ground_truth/ws_decoder_v3.txt", "w", encoding="utf-8").write("\n".join(out))
print("done")
