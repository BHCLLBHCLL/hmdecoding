...
for fname, info in gt.items():
    ...
    for (sh, segid, cfg71, cnt, X, Y) in segs:
        ...
        if X == 3:
            ...
            if got is None: notes.append(f"seg{segid} A-fail")
        else:
            got = parse_b_type(p, s, cnt, ncount, row_map, config, Y)
            if got is None: notes.append(f"seg{segid} B-fail")
        if got: elems.update(got)
    ...
    for eid, d in info["elems"].items():
        ...
        if eid in elems and elems[eid][1][:len(exp)] == exp and elems[eid][0] == d["cfg"]:
            ...
            wrong.append((eid, elems.get(eid)))
    print(f"== {fname}: segs={len(segs)} decoded={len(elems)} match={matches}/{total} {notes} wrong={wrong[:2]}")
