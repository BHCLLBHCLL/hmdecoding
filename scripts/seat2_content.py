...
oracle={}
for line in open('output/ground_truth/seat2_elems_all.txt',encoding='utf-8'):
...
print('seat_2 content: same_cfg=%d diff_cfg=%d same_nds=%d diff_nds=%d only_dec=%d only_ora=%d'%(same_cfg,diff_cfg,same_nds,diff_nds,only_dec,only_ora))
print('  dec distinct=%d oracle=%d dec records=%d'%(len(set(dec)),len(oracle),len(m.elements)))