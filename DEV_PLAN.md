...
| 几何 BREP（线/面/体） | 5% | 仅点；无线/面/体。路线: **STEP + Parasolid X_T 导入解析**
  (rail_extrusion.stp/beam.x_t 均为文本公开格式, 见 M4 路线决策); .hm 原生几何段解码降级低优先级 |
...
**HM 侧实测（关键发现）**: hmbatch `*writefile 任意扩展名` 统一输出 HM 几何序列化
（12B 前缀+gzip 同 .hm 容器, 含类型目录 CurveNURBS/SurfaceNURBS/AS_Face/AS_Body/AS_Part 等）;
原生 .hm 几何为紧凑二进制无类型名且坐标带变换, 逆向成本高。

**路线调整（2026-09）: 几何文件主要走 STEP 与 Parasolid X_T 导入, 不依赖上述 HM 导出序列化。**
案例库实证: tutorials/hm/rail_extrusion.stp = 标准 ISO-10303-21 文本;
tutorials/hwsolvers/optistruct/{beam,fluid}.x_t + mv_hv_hg/.../slotted_link.x_t = **文本
Parasolid Transmit 格式**（`**ABCDEFGHIJKLMNOPQRSTUVWXYZ...` 头 + `**PART1;` 实体行）——
两者均为**可直接解析的文本格式**（公开规范）。

**决策: 采用 STEP/X_T 导入→解析路线**: decoder 新增 `parse_step()` 与 `parse_x_t()`,
直接解析用户导入的 .stp/.step/.x_t 几何文件（非 .hm 原生几何、非 HM 导出序列化）:
- STEP (ISO-10303-21): CARTESIAN_POINT/DIRECTION/LINE/CIRCLE/B_SPLINE_CURVE_WITH_KNOTS/
  B_SPLINE_SURFACE/ADVANCED_FACE/EDGE_LOOP 等实体 → points/lines/surfs;
- X_T (Parasolid Transmit): 文本实体行 → 拓扑实体（对应 cabdecoding pskernel_user_guide 的
  X_T 流格式, PK_PART_receive 语义）;
- hm_gui 直接打开 .stp/.x_t（当前仅 .hm/.hmj）并渲染线框/着色面。
- .hm 原生几何段解码降级为低优先级（几何工作流以 CAD 导入为主）。

- [ ] 4.1a parse_step(): ISO-10303-21 解析器（点/线/圆/NURBS 曲线/曲面 + 面拓扑）→
      model.points/lines/surfs, oracle 对照（rail_extrusion.stp 等）
- [ ] 4.1b parse_x_t(): Parasolid Transmit 文本解析（拓扑实体 + NURBS 数据）→
      model.points/lines/surfs/solids, oracle 对照（beam.x_t/fluid.x_t/slotted_link.x_t）
- [x] 4.1c 案例诊断: WS_2.1_Geo 纯几何文件 0 节点/0 单元, oracle 148 点/243 线/79 面, 定位
      decoder 提取 0 的根因（parse_display_points 布局失效 + 无 lines/surfs 解码）
- [ ] 4.1d GUI 打开 .stp/.x_t + 线框/着色面渲染接线
- [ ] 4.1e .hm 原生几何段解码（低优先级, 以导出序列化为锚, 后续补）
