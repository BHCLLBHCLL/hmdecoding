...
- **oracle 对照（123 文件）**：node-ok **119/123 (97%)**，elem-ok **123/123 (100%)**；**--strict-nodes 模式 node-ok 123/123（100%）**；**--strict-elems 模式元素节点引用修正**（decode node_filter 过滤删除节点 — SEAT_MODEL/seatbelt 元素 same_nds 17374→27496/27503, +10122）；**内容级对照 strict-elems+elem_filter PERFECT 91/91**（eid 判别最终规则: rec12==0 一律 @+10 misaligned u32 (cartridge 全文件 +1 偏移, 首元素来自 family-1 尾段); v12∈(3,4) 去 u10 守卫 (yoke 262144); v12==1 misaligned (penetration 87155..); B 型 cfg55 MPC 变长记录 (channel_brkt seg3, [311][nslave][master][slave]×n) + B 型 cfg60 双节点链 (seg4, 114B 步长 next_eid 链) — 全部 91 文件逐元素 eid/config/节点全对）；**内容级对照（91 文件）PERFECT 68/91**（fe_only cfg220 + yoke 133792 + floor 2898 + body_side 20320 + rear_truss 1740 + abaqus3_0 270 + frame_assembly 10066 + truck cfg60 全对；cfg56 五节点 + cfg60 2节点修复） — 4 文件（seat_2/seat_start/truck/car_section）的 ±1 节点是 HM 运行时删除状态未持久化到字节流（无删除标记，纯解析不可识别），strict 模式用 oracle 节点 id 集合精确剔除残留，达成全 100% 对照 — all elements decoded! 全部元素全解（family-1 MPC config 22/55 变长记录 v11/v12 双布局 + 0x7050 分裂锚点 + v13 几何段 config-60；crash_tubes/abaqus3_0/truck/abaqus_contactManager_2D/seat_2/seat_start/hook/keyhole/joints/channel_brkt 全解；44B 链式节点乱值顺序补号；wing_section_complete 1001/1001、hm-ansys wizard_2d 202/202 全解（此前基线 node 66/98、elem 6/98）；v17 (17.01) 全解 dummy_positioner/seat_deformer 354174+585546；wing/wizard_2d/body_side_assembly/car_section/chapter2_2/manager_2d/abaqus_3D/hm-ansys_3d 全解；
...
- **节点坐标 content 级对照（新）**：hmbatch 导出 123 文件逐节点 id+xyz（`output/ground_truth/nc_all.log`），decode 逐节点比对。**修复两个真 bug**：(1) `parse_nodes` 坐标偏移 `xoff` 由布局决定 — `idoff==8` 布局坐标在 `@+20`（@+12 零填充），此前一律 12 读成 `(0,x,y)`（~150 万节点错）；(2) `find_node_section` 加**完整流长验证** — 真实节点头 count 条记录连续有效，假头（撞元素区）即断，修复 frame_assembly_1/2/3 误选 count=10784 假头（真 11836，此前丢 9971 节点）及 truck/seatbelt/seat_start 的 ±1 残留。**167 万节点仅剩 icw_ex1/2 链尾 33 坐标错 + car_section/seat_2 各 1-2**（±1 为 oracle 源差异：新 harvest vs corpus_gt）。
- **链式节点删除残留字节恢复**：56B 链 `@+44` 字段有跳号（删除节点 id 复用），跳号处（`raw > prev_raw+1`）记录 id 纠正为 `prev_raw`；parse_nodes 与 row_map 同步。**SEAT_MODEL/seatbelt 节点集 od/oo 0、元素节点列表 dn 10125→0**（此前误判为 HM 运行时状态未持久化，实为字节可恢复）。非 strict content PERFECT 75→77/91。
- **并行化**：`scripts/content_mp.py`（多进程 content 对照）、`scripts/gate_mp.py`（count 门禁）、`scripts/content_nf_mp.py`（非 strict 对照）— 91/123 文件并行解码，全量 content 对照加速；
